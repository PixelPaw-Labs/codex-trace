...
`Codex.Trace_aarch64.app.tar.gz` for macOS (unversioned by Tauri, and the only macOS
artifact — no `.dmg`, see `.github/workflows/release.yml`), plus five filenames stamped
with `$NEXT_VERSION` (Linux rpm / AppImage / deb, Windows exe / msi).
