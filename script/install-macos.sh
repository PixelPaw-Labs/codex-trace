...
# Install the pre-built Codex Trace desktop app on macOS without cloning the
# repo and without the usual `xattr -cr` dance.
#
...
#
# This installs the desktop app only. `codex-trace --web` is launched from a
# source checkout — see script/install.sh for that.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/PixelPaw-Labs/codex-trace/main/script/install-macos.sh | bash
#
# Environment overrides:
#   CODEXTRACE_VERSION      release tag to install (default: latest), e.g. v0.5.0
#   CODEXTRACE_INSTALL_DIR  destination directory (default: /Applications)
#   CODEXTRACE_BASE_URL     release download base (default: GitHub releases)

APP_NAME="Codex Trace"
BIN_NAME="codex-trace"
REPO="PixelPaw-Labs/codex-trace"
INSTALL_DIR="${CODEXTRACE_INSTALL_DIR:-/Applications}"
BASE_URL="${CODEXTRACE_BASE_URL:-https://github.com/${REPO}/releases}"
...
case "$arch" in
  arm64) asset="Codex.Trace_aarch64.app.tar.gz" ;;
  *)
    die "no pre-built app is published for ${arch} (Apple Silicon only).
Build from source instead:
  git clone https://github.com/${REPO}.git && cd codex-trace && ./script/install.sh"
    ;;
esac

if [[ -n "${CODEXTRACE_VERSION:-}" ]]; then
  url="${BASE_URL}/download/${CODEXTRACE_VERSION}/${asset}"
else
  url="${BASE_URL}/latest/download/${asset}"
fi

[[ -d "$INSTALL_DIR" ]] || die "install directory does not exist: $INSTALL_DIR"
[[ -w "$INSTALL_DIR" ]] || die "no write permission for $INSTALL_DIR. Re-run with sudo, or set CODEXTRACE_INSTALL_DIR to a writable location."
...
[[ -x "$staged/Contents/MacOS/${BIN_NAME}" ]] || die "unpacked bundle is missing its executable"

target="${INSTALL_DIR}/${APP_NAME}.app"
was_running=""
if pgrep -f "${APP_NAME}.app/Contents/MacOS/${BIN_NAME}" >/dev/null 2>&1; then
  was_running="yes"
fi
