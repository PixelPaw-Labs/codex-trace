...
use crate::commands::settings::{build_settings_response, SettingsResponse};
use crate::state::AppState;

/// Validate a single origin: must parse as an `http(s)://host[:port]` URI with
/// no path, query, or invalid header characters. Returns a message naming the
/// offending origin on failure.
fn validate_origin(origin: &str) -> Result<(), String> {
    ...
    // `"/"`, but a browser never sends a trailing slash in an `Origin` header —
    // so a stored `"http://example.com/"` would silently never match anything.
    // Reject it rather than accepting a value that can never work.
...
/// Trim, drop empty entries, de-duplicate (preserving order), and validate each
/// remaining entry as a well-formed CORS origin. Rejects the whole batch on the
/// first invalid entry rather than silently dropping it, mirroring how
/// `set_sessions_dir` rejects before mutating anything.
...
mod tests {
    ...
    fn rejects_non_http_scheme() {
        assert!(sanitize_and_validate_origins(vec!["file://example.com".to_string()]).is_err());
    }
    ...
    fn rejects_origin_with_query() {
        assert!(sanitize_and_validate_origins(vec!["http://example.com?a=1".to_string()]).is_err());
    }
    ...
    #[test]
    fn error_message_names_the_offending_origin() {
        let err = sanitize_and_validate_origins(vec!["nope".to_string()]).unwrap_err();
