 ...
 * the mode:
 *
 * - `codex-trace --web` (Vite on 1420, API on 11424): the `codextrace-api-token`
 *   plugin in `vite.config.ts` reads `clients/web-ui.jwt` and serves it as the
 *   virtual module imported below. `invoke.ts` sends it as an
 *   `X-CodexTrace-Token` header; `listen.ts` appends it as `?token=` because
 *   `EventSource` cannot set headers. When the file changes on disk (Reissue,
 *   from this tab or any other process) the plugin pushes the new value over
 *   HMR and the `import.meta.hot.accept` below swaps it in — no dev-server
 *   restart, no page reload.
 * - Docker (same-origin bundle): the virtual module is `""` in production
 *   builds. The server sets an HttpOnly cookie carrying the `web-ui` credential
 *   on the HTML shell, which the browser attaches automatically.
 ...
 */
import injected from "virtual:codextrace-api-token";
...
/**
 * Swap the live credential. Notifies subscribers only when the value actually
 * changes, so redundant sets (e.g. the HMR echo of a reissue this tab performed
 * itself) are free.
 */
...
export function authHeaders(): Record<string, string> {
  return token ? { "X-CodexTrace-Token": token } : {};
}
...
if (import.meta.hot) {
  ...
  import.meta.hot.accept("virtual:codextrace-api-token", (mod) => {
