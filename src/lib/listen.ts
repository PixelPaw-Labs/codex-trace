 ...
 * that could not happen; now it does whenever a reconnect is refused with a 401
 * — the browser reconnects with the URL (and, in Docker, whatever cookie) the
 * stream was opened with, so a backend restart, a sleep/wake or a suspended tab
 * that lands after the credential changed leaves the stream dead while the page
 * looks connected: no `session-update`, no `picker-refresh`, no live tail. So a
 * `CLOSED` stream is reopened here with the *current* credential, backing off
 * between attempts, for as long as anything is listening.
 */
...
function openSource(): EventSource {
  // `EventSource` can't set headers, so the client credential rides in the query
  // string (see lib/apiToken.ts). Empty in Docker, where the cookie is used.
  // `withCredentials` stays false: it only affects cross-origin requests, where
  // the cookie is not the carrier, and turning it on would make the browser
  // demand an `Access-Control-Allow-Credentials` the API does not send.
 ...
 * credential, re-attaching every registered listener. The old stream was
 * authenticated with the old credential and would silently die on its next
 * reconnect. No-op when nothing is listening.
 */
export function reconnectSse(): void {
  if (!sseSource) return;
  // A new credential is the likeliest cure for a refused stream: reopen now, and
  // let the next failure (if any) start the backoff from scratch.
  cancelReopen();
...
export async function listen<T>(
  ...
  return () => {
    // Remove from whichever connection is live now — it may have been replaced
    // by `reconnectSse` since this listener was attached.
