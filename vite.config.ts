 ...
 *
 * `codex-trace --web` serves the UI from Vite (port 1420) while the Rust API
 * listens on 11424, so the browser can't get the credential as a same-origin
 ...
 * as the virtual module `virtual:codextrace-api-token`, imported by
 ...
 *   codex-trace process), the module is invalidated and pushed over HMR.
 *   `apiToken.ts` accepts that update, so open tabs adopt the new credential in
 *   place. It must NOT restart the dev server: a restart makes Vite's client
 *   full-reload the page, which tears down the Settings modal the user just
 *   clicked in.
 */
const API_TOKEN_MODULE = "virtual:codextrace-api-token";
const API_TOKEN_MODULE_ID = `\0${API_TOKEN_MODULE}`;

function apiTokenPlugin(): Plugin {
  let serving = false;
  return {
    name: "codextrace-api-token",
    ...
    load(id) {
      ...
      // Re-read on every (re)load so an invalidation after a reissue serves the
    ...
    configureServer(server) {
      ...
      // `clients/` (nor, on a fresh install, the config root itself) exists yet
      // — the backend creates them moments after Vite starts, and chokidar only
      // reports a new file inside a directory it already watches.
      ...
      // Reissue replaces the file via rename, which some watchers report as add
      // rather than change.
